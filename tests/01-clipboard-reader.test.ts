...
import { assignWindow, pause } from './utils';
...
const evaluateTooManyCharactersMessage = async () => {
  const paragraphLoc = readerWindow.page.locator('.ichi-state-msg.too-many-characters')
...
const evaluateParseMessage = async () => {
  const paragraphLoc = readerWindow.page.locator('.parse-notification-msg')
  await paragraphLoc.waitFor({ timeout: 8000 });
  const paragraph = await paragraphLoc.evaluate((node: HTMLElement) => node.innerHTML)
  return paragraph
}
...
const evaluateSentence = async () => {
  const sentenceLoc = readerWindow.page.locator('.sentence-wrapper')
  await sentenceLoc.waitFor({ timeout: 8000 });
  const sentence = await sentenceLoc.evaluate((node: HTMLElement) => {
    // remove furigana
    let rt_elements = node.getElementsByTagName('rt');
    while (rt_elements[0]) rt_elements[0].parentNode?.removeChild(rt_elements[0])
    let rp_elements = node.getElementsByTagName('rp');
    while (rp_elements[0]) rp_elements[0].parentNode?.removeChild(rp_elements[0])
...
test.describe('Copy text to clipboard and test reader', () => {
  test('Wait until connected to ichi.moe', async () => {
    await readerWindow.page.locator('.ichi-state-msg.connected').waitFor({ timeout: 8000 })
  })

  passableTest('食べました', '食べました')
  passableTest('昨日の大雨。', '昨日の大雨。')
  passableTest('e昨日の大雨。e e', 'e昨日の大雨。ee')
