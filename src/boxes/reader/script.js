  ...
  const handleWords = (words) => {
    ...
    if (newWords.length == 0 && seenWords.length > 0) {
      ...
      seenWords.forEach((word) => {
        if (word.innerText != firstWord) isPlusOne = false;
      });
    } else if (seenWords.length == 0 && newWords.length > 0) {
    ...
    if (isPlusOne) document.querySelector("body").classList.add("plusOne");
